...
export default function GitaCommentaries() {
    ...
    return (
        <main>
            <div className="min-h-screen bg-black text-white p-4 md:p-10 pt-28 tracking-normal flex flex-row">
                <div className="-ml-10 w-1/5 pt-20">
                    <div className="bg-[#1C1C1C]/50 backdrop-blur-md border border-white/5 rounded-3xl p-6 h-[calc(100vh-200px)] flex flex-col">
                        <div className="text-xs font-bold text-gray-500 uppercase tracking-[0.2em] mb-6 text-center">Verses</div>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 overflow-y-auto pr-2 custom-scrollbar">
                            {verseKeys.map((vKey) => (
                                <button
                                    key={vKey}
                                    onClick={() => {
                                    ...
                                    className={`relative group aspect-square rounded-xl flex items-center justify-center text-sm font-bold transition-all duration-500 ${currentVerse === vKey
                                            ? 'bg-[#FFC226] text-black shadow-[0_0_20px_rgba(255,194,38,0.3)]'
                                            : 'bg-white/5 text-gray-500 hover:bg-white/10 hover:text-gray-200 border border-white/5'
                                        }`}
                                >
                                    {currentVerse === vKey && (
                                        <div className="absolute inset-0 bg-[#FFC226] blur-md opacity-20 rounded-xl"></div>
                                    )}
