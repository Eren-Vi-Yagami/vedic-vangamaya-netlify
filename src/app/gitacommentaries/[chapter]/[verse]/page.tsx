...
export default function GitaCommentaries() {
    ...
    // Use currentChapter and currentVerse from URL params
    ...
    const navigateTo = (chap: string, ver: string) => {
        router.push(`/gitacommentaries/${chap}/${ver}`);
        setOpenCommentaryIndex(0); // Reset commentary accordion on navigation
    };
                            ...
                            <div className="grid grid-cols-6 sm:grid-cols-10 lg:grid-cols-4 gap-2 overflow-y-auto pr-1 custom-scrollbar flex-grow">
                                {verseKeys.map((vKey) => (
                                    <button
                                        key={vKey}
                                        onClick={() => {
                                            navigateTo(currentChapter, vKey);
                                        }}
