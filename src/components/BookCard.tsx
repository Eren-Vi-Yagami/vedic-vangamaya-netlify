 ...
 */
export default function BookCard({ id, title, author, coverUrl, slug, status, category }: BookCardProps) {
  return (
    <Link
      href={`/books/${slug}`}
    ...
    >
      <div className="bg-white/10 backdrop-blur-lg rounded-xl border border-white/20 p-4 shadow-lg hover:shadow-2xl transition-all duration-300 group-hover:border-veda-accent/50">
        {coverUrl && (
        ...
        <h3 className="text-xl font-semibold text-white truncate group-hover:text-veda-accent transition-colors">
          {title}
        </h3>
